...
查找 `cursor-agent` 与 `agent`, 用 `--version` / `--help` 核实 Cursor Agent 身份后保存绝对入口路径. `cursor` 通常启动编辑器, 同名 `agent` 可能属于其他产品.
...
成功结果只表示该轮已正常返回, 修改任务以实际 diff 与独立验证为准. 超时或取消不回滚文件, 也不保证取消轮文本持久化: 先核对进程和工作区, 恢复时重述关键要求. 协议失联时终止本次拥有的进程树, 保留输出与会话 ID; 不重复可能已经发生的外部操作.
